...
- Name: recovery.spec.ts >> Journal page >> renders journal with actions
- Location: e2e\recovery.spec.ts:81:3
...
Locator: locator('.hero-title, .title').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.hero-title, .title').first()

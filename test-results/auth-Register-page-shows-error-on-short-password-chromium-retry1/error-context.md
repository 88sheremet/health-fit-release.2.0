...
- Name: auth.spec.ts >> Register page >> shows error on short password
- Location: e2e\auth.spec.ts:109:3

# Error details

```
Error: locator.fill: Error: strict mode violation: locator('input[type="password"]') resolved to 2 elements:
    1) <input value="" tabindex="0" type="password" aria-label="Пароль" autocomplete="new-password" class="q-field__native q-placeholder" id="f_6e5cc37c-8371-44b0-8dff-13c80b25f05e"/> aka getByRole('textbox', { name: 'Пароль', exact: true })
    2) <input value="" tabindex="0" type="password" autocomplete="new-password" aria-label="Повторите пароль" class="q-field__native q-placeholder" id="f_0d0dee80-b9fa-4c95-896f-c7aeb6c797c2"/> aka getByRole('textbox', { name: 'Повторите пароль' })

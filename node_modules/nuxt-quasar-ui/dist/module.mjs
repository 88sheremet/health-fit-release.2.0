...
function virtualAnimationsPlugin({ options, resolveQuasarExtras, quasarExtrasGte2 }) {
  return {
    ...
    async load(id) {
      ...
      if (animations === "all") {
        const animateListPath = resolveQuasarExtras(
          quasarExtrasGte2 ? "animate/animate-list.common" : "animate/animate-list.mjs"
        );
        const { generalAnimations, inAnimations, outAnimations } = await import(pathToFileURL(animateListPath).href);
...
const module$1 = defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    const resolveQuasarExtras = (...paths) => resolvePathSync(join("@quasar/extras", ...paths), { url: resolveURLs });
